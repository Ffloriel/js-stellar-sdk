import axios from 'axios';
import { when } from 'jest-when'
import StellarSdk, { HorizonAxiosClient } from './../../src/index'
import { Server } from '../../src/server';
import { Transaction, Memo, MemoType, Operation } from 'stellar-base';

const horizonAxiosClientMock = HorizonAxiosClient as jest.Mocked<typeof HorizonAxiosClient>;
horizonAxiosClientMock.post = jest.fn();

describe('server.js transaction tests', () => {
  let server: Server;
  let transaction: Transaction<Memo<MemoType>, Operation[]>;
  let blob: string;
  beforeEach(() => {
    server = new StellarSdk.Server('https://horizon-live.stellar.org:1337');
    StellarSdk.Config.setDefault();
    StellarSdk.Network.useTestNetwork();

    const keypair = StellarSdk.Keypair.random();
    const account = new StellarSdk.Account(keypair.publicKey(), '56199647068161');
    const transactionBuilt = new StellarSdk.TransactionBuilder(account, { fee: 100 })
      .addOperation(
        StellarSdk.Operation.payment({
          destination:
            'GASOCNHNNLYFNMDJYQ3XFMI7BYHIOCFW3GJEOWRPEGK2TDPGTG2E5EDW',
          asset: StellarSdk.Asset.native(),
          amount: '100.50'
        })
      )
      .setTimeout(StellarSdk.TimeoutInfinite)
      .build();
    transactionBuilt.sign(keypair);
    transaction = transactionBuilt;

    blob = encodeURIComponent(
      transaction
        .toEnvelope()
        .toXDR()
        .toString('base64')
    );
  });

  it('sends a transaction', async () => {
    when(horizonAxiosClientMock.post)
      .calledWith('https://horizon-live.stellar.org:1337/transactions', `tx=${blob}`)
      .mockResolvedValue({
        data: {},
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {}
      });

    expect.assertions(1);
    expect(() => { server.submitTransaction(transaction) }).not.toThrow();
  });
  it('adds metadata - tx was too small and was immediately deleted', async () => {
    const data = {
      _links: {
        transaction: {
          href:
            'https://horizon.stellar.org/transactions/db2c69a07be57eb5baefbfbb72b95c7c20d2c4d6f2a0e84e7c27dd0359055a2f'
        }
      },
      hash: 'db2c69a07be57eb5baefbfbb72b95c7c20d2c4d6f2a0e84e7c27dd0359055a2f',
      ledger: 22895637,
      envelope_xdr:
        '',
      result_xdr: 'AAAAAAAAAGQAAAAAAAAAAQAAAAAAAAADAAAAAAAAAAAAAAACAAAAAA==',
      result_meta_xdr:
        ''
    };

    when(horizonAxiosClientMock.post)
      .calledWith('https://horizon-live.stellar.org:1337/transactions', `tx=${blob}`)
      .mockResolvedValue({
        data,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {}
      });

    expect.assertions(10);
    const transactionResponse = await server.submitTransaction(transaction);
    const firstOfferResult = transactionResponse.offerResults![0];
    expect(Array.isArray(transactionResponse.offerResults)).toBe(true);
    expect(Array.isArray(firstOfferResult.offersClaimed)).toBe(true);
    expect(typeof firstOfferResult.effect).toBe('string');
    expect(firstOfferResult.wasImmediatelyFilled).toBe(false);
    expect(firstOfferResult.wasImmediatelyDeleted).toBe(true);
    expect(firstOfferResult.wasPartiallyFilled).toBe(false);

    expect(firstOfferResult.operationIndex).toBe(0);
    expect(firstOfferResult.amountBought).toBe('0');
    expect(firstOfferResult.amountSold).toBe('0');
    expect(firstOfferResult.currentOffer).toBe(undefined);

  });

  it('adds metadata, order immediately fills', async () => {
    const data = {
      _links: {
        transaction: {
          href:
            'https://horizon.stellar.org/transactions/d88ded94c558790f7e819b85fd35adb10a1e474312c34ebd611495c349a8eb69'
        }
      },
      hash: 'd88ded94c558790f7e819b85fd35adb10a1e474312c34ebd611495c349a8eb69',
      ledger: 22895558,
      envelope_xdr:
        '',
      result_xdr:
        'AAAAAAAAAGQAAAAAAAAAAQAAAAAAAAADAAAAAAAAAAEAAAAAdZtwPxUrSKeMmz4rsGwKlBWXVRNIbTx3gJgwrvXYkfwAAAAABGaF7AAAAAFCQVQAAAAAAEZK09vHmzOmEMoVWYtbbZcKv3ZOoo06ckzbhyDIFKfhAAAAAACj1/kAAAAAAAAAAAExLP8AAAACAAAAAA==',
      result_meta_xdr:
        ''
    };

    when(horizonAxiosClientMock.post)
      .calledWith('https://horizon-live.stellar.org:1337/transactions', `tx=${blob}`)
      .mockResolvedValue({
        data,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {}
      });

    expect.assertions(8);
    const transactionResponse = await server.submitTransaction(transaction);
    const firstOfferResult = transactionResponse.offerResults![0];
    expect(Array.isArray(transactionResponse.offerResults)).toBe(true);
    expect(Array.isArray(firstOfferResult.offersClaimed)).toBe(true);
    expect(typeof firstOfferResult.effect).toBe('string');
    expect(firstOfferResult.wasImmediatelyFilled).toBe(true);
    expect(firstOfferResult.wasImmediatelyDeleted).toBe(false);
    expect(firstOfferResult.wasPartiallyFilled).toBe(false);

    expect(firstOfferResult.operationIndex).toBe(0);
    expect(firstOfferResult.amountSold).toBe('1.9999999');
  });

  it('adds metadata, order is open', async () => {
    const data = {
      _links: {
        transaction: {
          href:
            'https://horizon.stellar.org/transactions/e1c2b91141d8c4185dc8c18118f345a269d88c476bdadec695c1b3ecdc999831'
        }
      },
      hash: 'e1c2b91141d8c4185dc8c18118f345a269d88c476bdadec695c1b3ecdc999831',
      ledger: 22896129,
      envelope_xdr:
        '',
      result_xdr:
        'AAAAAAAAAGQAAAAAAAAAAQAAAAAAAAADAAAAAAAAAAAAAAAAAAAAAIUAEW3jQt3+fbT6nCASA1/8RWdp9fJ2woxqPHZPQUH/AAAAAARmp8YAAAAAAAAAAUJBVAAAAAAARkrT28ebM6YQyhVZi1ttlwq/dk6ijTpyTNuHIMgUp+EAAAAAAJiWgAAAAAEAAAABAAAAAAAAAAAAAAAA',
      result_meta_xdr:
        ''
    };

    when(horizonAxiosClientMock.post)
      .calledWith('https://horizon-live.stellar.org:1337/transactions', `tx=${blob}`)
      .mockResolvedValue({
        data,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {}
      });

    expect.assertions(9);
    const transactionResponse = await server.submitTransaction(transaction);
    const firstOfferResult = transactionResponse.offerResults![0];
    expect(Array.isArray(transactionResponse.offerResults)).toBe(true);
    expect(Array.isArray(firstOfferResult.offersClaimed)).toBe(true);
    expect(typeof firstOfferResult.effect).toBe('string');
    expect(firstOfferResult.wasImmediatelyFilled).toBe(false);
    expect(firstOfferResult.amountBought).toBe('0');
    expect(firstOfferResult.wasImmediatelyDeleted).toBe(false);
    expect(firstOfferResult.wasPartiallyFilled).toBe(false);
    expect(firstOfferResult.isFullyOpen).toBe(true);
    expect(firstOfferResult.operationIndex).toBe(0);
  });

  it('adds metadata, partial fill', async () => {
    const data = {
      _links: {
        transaction: {
          href:
            'https://horizon.stellar.org/transactions/28552ba6a70ab74f6de05319950e2ddad94491159ebc97b14cfcde2d3c7e70a1'
        }
      },
      hash: '28552ba6a70ab74f6de05319950e2ddad94491159ebc97b14cfcde2d3c7e70a1',
      ledger: 22896525,
      envelope_xdr:
        '',
      result_xdr:
        '',
      result_meta_xdr:
        ''
    };

    when(horizonAxiosClientMock.post)
      .calledWith('https://horizon-live.stellar.org:1337/transactions', `tx=${blob}`)
      .mockResolvedValue({
        data,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {}
      });

    expect.assertions(14);
    const transactionResponse = await server.submitTransaction(transaction);
    const firstOfferResult = transactionResponse.offerResults![0];
    expect(Array.isArray(transactionResponse.offerResults)).toBe(true);
    expect(Array.isArray(firstOfferResult.offersClaimed)).toBe(true);
    expect(firstOfferResult.offersClaimed.length).toBe(1);
    expect(firstOfferResult.effect).toBe('manageOfferCreated');
    expect(typeof firstOfferResult.effect).toBe('string');
    expect(firstOfferResult.wasImmediatelyFilled).toBe(false);
    expect(firstOfferResult.amountBought).toBe('53.8299095');
    expect(firstOfferResult.wasImmediatelyDeleted).toBe(false);
    expect(firstOfferResult.wasPartiallyFilled).toBe(true);
    expect(firstOfferResult.isFullyOpen).toBe(false);
    expect(firstOfferResult.operationIndex).toBe(0);
    expect(firstOfferResult.currentOffer!.selling.type).toBe('native');
    expect(firstOfferResult.currentOffer!.buying.assetCode).toBe('BAT');
    expect(firstOfferResult.currentOffer!.buying.issuer).toBe('GBDEVU63Y6NTHJQQZIKVTC23NWLQVP3WJ2RI2OTSJTNYOIGICST6DUXR');
  });

  it('doesnt add metadata to non-offers', async () => {
    const data = {
      _links: {
        transaction: {
          href:
            'https://horizon.stellar.org/transactions/6c3191f252f2c586c74275c766ce761021513e520eab3bb63d3fd18d0d01492e'
        }
      },
      hash: '6c3191f252f2c586c74275c766ce761021513e520eab3bb63d3fd18d0d01492e',
      ledger: 22893969,
      envelope_xdr:
        '',
      result_xdr: 'AAAAAAAAAGQAAAAAAAAAAQAAAAAAAAAGAAAAAAAAAAA=',
      result_meta_xdr:
        ''
    };

    when(horizonAxiosClientMock.post)
      .calledWith('https://horizon-live.stellar.org:1337/transactions', `tx=${blob}`)
      .mockResolvedValue({
        data,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {}
      })

    expect.assertions(1);
    const transactionResponse = await server.submitTransaction(transaction);
    expect(transactionResponse.offerResults).toBe(undefined);
  });

  it('adds metadata about offers, even if some ops are not', async () => {
    const data = {
      _links: {
        transaction: {
          href:
            'https://horizon.stellar.org/transactions/6a22d6896140f6f330ef19086827df0780eb2ad3324f3271b38c70cb1cba1c3d'
        }
      },
      hash: '6a22d6896140f6f330ef19086827df0780eb2ad3324f3271b38c70cb1cba1c3d',
      ledger: 22894978,
      envelope_xdr:
        '',
      result_xdr:
        'AAAAAAAAAZAAAAAAAAAABAAAAAAAAAAGAAAAAAAAAAAAAAAFAAAAAAAAAAAAAAADAAAAAAAAAAAAAAACAAAAAAAAAAMAAAAAAAAAAAAAAAIAAAAA',
      result_meta_xdr:
        ''
    };

    when(horizonAxiosClientMock.post)
      .calledWith('https://horizon-live.stellar.org:1337/transactions', `tx=${blob}`)
      .mockResolvedValue({
        data,
        status: 200,
        statusText: 'OK',
        headers: {},
        config: {}
      });

    expect.assertions(8);
    const transactionResponse = await server.submitTransaction(transaction);
    const firstOfferResult = transactionResponse.offerResults![0];
    const secondOfferResult = transactionResponse.offerResults![1];
    expect(Array.isArray(transactionResponse.offerResults)).toBe(true);
    expect(transactionResponse.offerResults!.length).toBe(2);
    expect(Array.isArray(firstOfferResult.offersClaimed)).toBe(true);
    expect(typeof firstOfferResult.effect).toBe('string');
    expect(firstOfferResult.operationIndex).toBe(2);
    expect(Array.isArray(secondOfferResult.offersClaimed)).toBe(true);
    expect(typeof secondOfferResult.effect).toBe('string');
    expect(secondOfferResult.operationIndex).toBe(3);
  });
});
