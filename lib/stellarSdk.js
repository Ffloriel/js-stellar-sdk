...
var URI$1 = _interopDefault(require('urijs'));
...
(function (root, factory) {
  ...
  if (module.exports) {
    // Node
    module.exports = factory(URI);
  } else {
    // Browser globals (root is window)
    root.URITemplate = factory(root.URI, root);
  }
}(commonjsGlobal, function (URI, root) {
  // FIXME: v2.0.0 renamce non-camelCase properties to uppercase
  /*jshint camelcase: false */
...
function interceptorHorizonResponse(response) {
    const hostname = URI$1(response.config.url).hostname();
...
class CallBuilder {
    ...
    _toCollectionPage(json) {
        ...
        return {
            ...
            next: () => this._sendNormalRequest(URI$1(json._links.next.href)).then((r) => this._toCollectionPage(r)),
            prev: () => this._sendNormalRequest(URI$1(json._links.prev.href)).then((r) => this._toCollectionPage(r))
        };
    }
...
class AccountResponse {
    constructor(response) {
        this.baseAccount = new stellarBase.Account(response.account_id, response.sequence);
...
class TransactionCallBuilder extends CallBuilder {
    ...
    forLedger(sequence) {
        const ledgerSequence = typeof sequence === 'number' ? sequence.toString() : sequence;
...
class Server {
    constructor(serverURL, opts = {}) {
        this.serverURL = URI$1(serverURL);
    ...
    async submitTransaction(transaction) {
        ...
        try {
            ...
            const responseXDR = stellarBase.xdr.TransactionResult.fromXDR(response.data.result_xdr, 'base64');
            ...
            if (results.length) {
                offerResults = results.map((result, i) => {
                    ...
                    const offersClaimed = offerSuccess.offersClaimed()
                        ...
                        const sold = stellarBase.Asset.fromOperation(offerClaimed.assetSold());
                        const bought = stellarBase.Asset.fromOperation(offerClaimed.assetBought());
                        const assetSold = {
                        ...
                        return {
                            sellerId: stellarBase.StrKey.encodeEd25519PublicKey(offerClaimed.sellerId().ed25519()),
                    ...
                    if (typeof offerSuccess.offer().value === 'function' &&
                        ...
                        const selling = stellarBase.Asset.fromOperation(offerXDR.selling());
                        currentOffer.selling = {
                            type: selling.getAssetType(),
                            assetCode: selling.getCode(),
                            issuer: selling.getIssuer()
                        };
                        const buying = stellarBase.Asset.fromOperation(offerXDR.buying());
                        currentOffer.buying = {
...
class FederationServer {
    ...
    static async resolve(value, opts = {}) {
        ...
        if (value.indexOf('*') < 0) {
            if (!stellarBase.StrKey.isValidEd25519PublicKey(value)) {
...
exports.HorizonAxiosClient = HorizonAxiosClient;
